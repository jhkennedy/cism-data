...
nc = Dataset('antarctica_500m.nc', 'w', format='NETCDF4')
...
bm2_xvals = arange(x0,x0+nx*dx,dx*2)
bm2_yvals = arange(y0,y0+ny*dy,dy*2)
bm2nx = len(bm2_xvals)
bm2ny = len(bm2_yvals)
...
buf = asarray( unpack( str(bm2nx*bm2ny)+'f' , f.read() ) )
f.close()
buf[buf==-9999.] = 0.
f = interp2d( bm2_yvals[::strd] , bm2_xvals[::strd] , buf.reshape((bm2ny,bm2nx))[::strd,::strd] , kind='linear' , copy=False , bounds_error=False, fill_value=0. )
thk_v[:,:]  = f( y_v[:] , x_v[:] )[::-1,:]
...
nc_v = Dataset('originals/Rignot-InSAR-2011/450m/antarctica_ice_velocity_450m.nc','r')
strd=1
