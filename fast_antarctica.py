...
nco = Nco()
nco.ncks(input='antarctica_1km_2017_05_03.nc', output=f_1km, options=['-x', '-v', 'acab_alb,artm_alb'])
nco.ncrename(input=f_1km, output=f_1km, options=['-v', 'dzdt,dHdt'])
nco.ncrename(input=f_1km, output=f_1km, options=['-v', 'verr,vErr'])
...
if FILL:
    nc_new.variables['vErr'][0, :, :] = verr[:, :].filled(0.0)
else:
...
nc_new.variables['dHdt'].reference = "Mcmillan, M., A. Shepherd, A. Sundal, K. Briggs, A. Muir, A. Ridout, A. Hogg, and D. Wingham, 2014: Increased ice losses from Antarctica detected by CryoSat-2. Geophys. Res. Lett, doi:10.1002/(ISSN)1944-8007."
