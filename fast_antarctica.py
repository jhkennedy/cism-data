...
berr_thkerr = np.ma.masked_invalid(nc_berr.variables['thkerr'][::-1, :])
# berr_to_new = scipy.interpolate.RectBivariateSpline(berr.y[:], berr.x[:], berr_thkerr.filled(fill_value=berr_thkerr.max()), kx=1, ky=1, s=0)
# new_berr = np.zeros(new.dims)
# for ii in range(0, new.nx):
#     new_berr[:, ii] = berr_to_new.ev(new.y_grid[:, ii], new.x_grid[:, ii])
...
nc_new.variables['thkerr'].comments = 'Resampled from 5km grid using 2D nearest neighbor interpolation'
# FIXME nc_new.variables['thkerr'].missing_value = FIXME
...
nc_new.variables['bheatflx'][0, :, :] = -new_bheatflx[:, :]  # invert sign for model convention
nc_new.variables['bheatflx'].long_name = "Basal Heat Flux"
nc_new.variables['bheatflx'].standard_name = "downward_geothermal_flux_at_ice_base"
...
nc_new.variables['bheatflxerr'][0, :, :] = -new_bheatflxerr[:, :]  # invert sign for model convention
...
nc_new.variables['time'].commemt = 'The initial time here is an estimate of the nominal date for Rignot (2011) InSAR velocity data. Because this is a synthesis of datasets across many time periods, the inital date is inherently fuzzy and should be changed to suit your purposes.'

nc_new.variables['y1'].long_name = 'y-coordinate in projection'
nc_new.variables['y1'].standard_name = 'projection_y_coordinate'
nc_new.variables['y1'].axis = 'Y'
nc_new.variables['y1'].units = 'm'

nc_new.variables['x1'].long_name = 'x-coordinate in projection'
nc_new.variables['x1'].standard_name = 'projection_x_coordinate'
nc_new.variables['x1'].axis = 'X'
nc_new.variables['x1'].units = 'm'

nc_new.createVariable('epsg_3031', 'b')
...
nc_new.variables['epsg_3031'].scale_factor = "1"
nc_new.variables['epsg_3031'].false_easting = "0"
nc_new.variables['epsg_3031'].false_northing = "0"
nc_new.variables['epsg_3031'].ellipsoid = "WGS84"
nc_new.variables['epsg_3031'].datum = "WGS84"
